...
1. Download the complete v1.4 image from
   [beebem.webassembly.link](https://beebem.webassembly.link/flash/beebem-esp32-s3-v1.4.bin).
...
4. On macOS, this single block finds the first attached USB modem port,
   ...
   curl --fail --location \
     https://beebem.webassembly.link/flash/beebem-esp32-s3-v1.4.bin --output "$IMAGE"
...
This produces `dist/beebem-esp32-s3-v1.4.bin` plus a SHA-256 file. Both `dist/`
...
Its physical controls use **PWR** for Space and **BOOT** for Return/jump; the
centre touch action also sends Return/jump.
