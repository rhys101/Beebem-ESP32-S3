...
static void *disc_calloc(size_t count, size_t size) {
  const size_t bytes = count * size;
  void *memory = heap_caps_malloc(bytes, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  if (memory != NULL) memset(memory, 0, bytes);
  return memory;
}

static void *disc_malloc(size_t size) {
  return heap_caps_malloc(size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
}
#else
#define disc_calloc calloc
#define disc_malloc malloc
#endif
...
/* SSD loader for targets without a conventional filesystem. The representation
 * deliberately matches LoadSimpleDiscImage so the 8271 command path remains
 * untouched and host/device tests exercise identical code. */
int LoadSimpleDiscImageFromBuffer(const unsigned char *data, unsigned int size,
                                  int DriveNum, int HeadNum) {
  if (data == NULL || DriveNum < 0 || DriveNum > 1 || HeadNum < 0 || HeadNum > 1 ||
      size < 2 * 256 || (size % 256) != 0) return 0;

  /* Archive SSDs are sometimes truncated immediately after their last used
   * sector even though the DFS catalogue still describes an 80-track disc.
   * LoadSimpleDiscImage's fread/calloc path naturally zero-fills the omitted
   * tail. Do the same for embedded images instead of requiring a whole-track
   * file, and retain enough tracks for the catalogue's declared capacity. */
  const unsigned int image_sectors = (size + 255) / 256;
  const unsigned int catalogue_sectors =
      ((unsigned int)(data[0x106] & 3) << 8) | data[0x107];
  unsigned int tracks = (image_sectors + 9) / 10;
  const unsigned int catalogue_tracks = (catalogue_sectors + 9) / 10;
  if (catalogue_tracks > tracks) tracks = catalogue_tracks;
  if (tracks > TRACKSPERDRIVE) return 0;

  FreeDiscImage(DriveNum);
  NumHeads[DriveNum] = 1;
  snprintf(FileNames[DriveNum], sizeof(FileNames[DriveNum]), "embedded.ssd");

  unsigned int offset = 0;
  for (unsigned int track = 0; track < tracks; ++track) {
    TrackType *track_ptr = &DiscStore[DriveNum][HeadNum][track];
    track_ptr->LogicalSectors = 10;
    track_ptr->NSectors = 10;
    track_ptr->Sectors = (SectorType *)disc_calloc(10, sizeof(SectorType));
    if (track_ptr->Sectors == NULL) {
      FreeDiscImage(DriveNum);
      return 0;
    }
    track_ptr->Gap1Size = track_ptr->Gap3Size = track_ptr->Gap5Size = 0;

    for (unsigned int sector = 0; sector < 10; ++sector) {
      SectorType *sector_ptr = &track_ptr->Sectors[sector];
      sector_ptr->IDField.CylinderNum = track;
      sector_ptr->IDField.RecordNum = sector;
      sector_ptr->IDField.HeadNum = HeadNum;
      sector_ptr->IDField.PhysRecLength = 256;
      sector_ptr->Deleted = 0;
      sector_ptr->Data = (unsigned char *)disc_malloc(256);
      if (sector_ptr->Data == NULL) {
        FreeDiscImage(DriveNum);
        return 0;
      }
      if (offset < size) {
        memcpy(sector_ptr->Data, data + offset, 256);
      } else {
        memset(sector_ptr->Data, 0, 256);
      }
      offset += 256;
    }
  }
...
int Disc8271_set_writable_path(int DriveNum, const char *path) {
  if (DriveNum < 0 || DriveNum > 1 || path == NULL || path[0] == '\0' ||
      DiscStore[DriveNum][0][0].Sectors == NULL) return 0;
  snprintf(FileNames[DriveNum], sizeof(FileNames[DriveNum]), "%s", path);
  Writeable[DriveNum] = 1;
  return 1;
}
