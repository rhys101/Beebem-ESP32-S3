...
char TeletextStyle=2; // Half-height Mode 7 raster for the 256-line framebuffer.
int THalfMode=1; // Render alternate source rows so all 25 rows fit in 250 lines.
int CurY=-1;
FILE *crtclog;
...
static const unsigned char *Mode7FontData;
static unsigned int Mode7FontDataSize;
static VideoFrameCompleteCallback FrameCompleteCallback;
static void *FrameCompleteContext;

void VideoSetTeletextFont(const unsigned char *font, unsigned int size) {
  Mode7FontData = font;
  Mode7FontDataSize = size;
}

void VideoSetFrameCompleteCallback(VideoFrameCompleteCallback callback, void *context) {
  FrameCompleteCallback = callback;
  FrameCompleteContext = context;
}
...
static void BuildMode7FontHandheld(void) {
	...
	for(m7cc=0; m7cc<96; m7cc++) {
		for(m7cy=0; m7cy<9; m7cy++) {
			...
			for(m7cx=0; m7cx<8; m7cx++) {
				const char *font_row = mode7fontRaw[(m7cc / 16) * 9 + m7cy];
				p = font_row[(m7cc % 16) * 8 + m7cx] != ' ';
				if (p) {
					//printf("*");
...
/* Restore BeebEm's full 12x20 Mode 7 font for the 640-pixel logical surface. */
static void BuildMode7Font(void) {
  unsigned int m7cc, m7cy;
  memset(EM7Font, 0, sizeof(EM7Font));

  if (Mode7FontData != NULL && Mode7FontDataSize == 96 * 18 * 2) {
    for (m7cc = 0; m7cc < 96; ++m7cc) {
      for (m7cy = 0; m7cy < 18; ++m7cy) {
        const unsigned int offset = (m7cc * 18 + m7cy) * 2;
        const unsigned int row =
            (Mode7FontData[offset] | (Mode7FontData[offset + 1] << 8)) << 2;
        EM7Font[0][m7cc][m7cy + 2] = row;
        EM7Font[1][m7cc][m7cy + 2] = row;
        EM7Font[2][m7cc][m7cy + 2] = row;
      }
    }
  } else {
    BuildMode7FontHandheld();
    return;
  }

  for (m7cc = 0; m7cc < 96; ++m7cc) {
    if (!(m7cc & 32)) {
      unsigned int row1 = 0, row2 = 0, row3 = 0;
      if (m7cc & 1) row1 |= 4032;
      if (m7cc & 2) row1 |= 63;
      if (m7cc & 4) row2 |= 4032;
      if (m7cc & 8) row2 |= 63;
...
#	define SYNC 40
	++FrameCount;
	if (!FrameNum && FrameCompleteCallback != NULL) {
		FrameCompleteCallback(FrameCompleteContext);
	}
//#endif
...
{
		// Flipped mode
		if (line < 0 || line >= FRAME_HEIGHT) return NULL;
		if ( IsFlippedMode() ) line = FRAME_HEIGHT - 1 - line;
...
//	printf("***%d\n", scanline);
	...
	if (CursorO) {
		if (CursorX < 0 || CursorX + CursorW >= FRAME_WIDTH) return;
			
		for(int y=0; y<CursorH; y++) {
			p = GetTeletextScanlinePtr((CursorY+ /*32+*/ y+14) / TeletextStyle);
			if (p == NULL) continue;

			if ( IsFlippedMode() ) {
				p+= FRAME_WIDTH - 1 - CursorX;
				for(int x=0; x<CursorW; x++) {
					//*(p--) ^= mainWin->cols[7];
					*(p--)= mainWin->cols[7];
				}
...
static inline void DrawLine(long Col, int y, int sx, int width)
{
	...
	if (sx+width >=FRAME_WIDTH) width = FRAME_WIDTH-sx;

	// Flipped mode
	if ( IsFlippedMode() ) sx = FRAME_WIDTH - sx - width;
...
static inline unsigned char* GetScanlinePtr(int line)
{
	...
	/* Keep all 256 BBC scanlines; the panel scaler handles presentation. */
	scanline = line;
	if (mainWin->OnScreenKeyboardShown() && scanline >= 105) return NULL;

//	scanline = line;

	/* Flip it for flipped mode:
 	 */
	if ( IsFlippedMode() ) scanline = abs( (FRAME_HEIGHT - 1) - ((long)scanline) );
...
//	if ( IsFlippedMode() ) p+= 319;
	if ( IsFlippedMode() ) p+= FRAME_WIDTH;
...
static void DoMode7Row(void) {
  ...
  int CurrentScanLine;
  const int ContentWidth=CRTC_HorizontalDisplayed*CHAR_WIDTH;
  const int Mode7XOffset=ContentWidth<FRAME_WIDTH?(FRAME_WIDTH-ContentWidth)/2:0;
  int CurrentX=Mode7XOffset;
  int CurrentPixel;
  unsigned int col;
...
//		DrawLine(unsigned char Col, int y, int sx, int width)
            ...
            DrawLine(CurrentCol[CurrentScanLine],
                     (VideoState.PixmapLine+CurrentScanLine+TELETEXT_Y_OFFSET) / TeletextStyle,
...
//      mainWin->doHorizLine(CurrentCol[CurrentScanLine],VideoState.PixmapLine+CurrentScanLine,CurrentStartX[CurrentScanLine],CurrentLen[CurrentScanLine]);
    ...
    /* Mode 7 has a 480-pixel active raster. Keep its unused sides black and
       centre that raster in the common 640-pixel BBC framebuffer. */
    const int y=(VideoState.PixmapLine+CurrentScanLine+TELETEXT_Y_OFFSET) / TeletextStyle;
    if (Mode7XOffset>0) {
      DrawLine(0,y,0,Mode7XOffset);
      DrawLine(0,y,Mode7XOffset+ContentWidth,FRAME_WIDTH-Mode7XOffset-ContentWidth);
    }
...
#ifdef ESP_PLATFORM
	// Keep BBC timing at 50 Hz but only paint alternate fields. A full
	// 640x256 framebuffer in external RAM is deliberately large enough for all
	// modes; rendering at the AMOLED's useful 25 Hz rate avoids doing that
	// bandwidth-heavy work twice for every displayed frame.
	FrameNum = FrameCount & 1U;
#else
	FrameNum=0;
#endif
  ...
  /* Clear the next 8 scan lines */
    if (!FrameNum)
	      ...
	      if (/* VScreenAdjust>0 && */ VideoState.PixmapLine==0) {
			...
			for (l=0; l<7; l++) {
				DrawLine(0, l, 0, FRAME_WIDTH);
				if (! mainWin->OnScreenKeyboardShown()) DrawLine(0, FRAME_HEIGHT-1-l, 0, FRAME_WIDTH);
		}
		// 7 lines at top, 8 lines at bottom. This is the extra one for the bottom (above loop 0-6)
		if (! mainWin->OnScreenKeyboardShown()) DrawLine(0, FRAME_HEIGHT-1-7, 0, FRAME_WIDTH);
	}
...
//	        mainWin->updateLines(0,(500/TeletextStyle));
	...
	if (!FrameNum) AddTeletextHardwareCursor(VideoState.PixmapLine);
...
//$$	    memset(mainWin->GetLinePtr(VideoState.PixmapLine),200,800);
    ...
    if (VideoState.CharLine!=-1) {
      if (VideoState.CharLine<CRTC_VerticalDisplayed) {
		...
		if (!FrameNum) ClearScanline(VideoState.PixmapLine);
...
//+>
	// Adjust to scale:
	// Keep native BBC horizontal resolution; scaling occurs at panel output.
//<+
}
*/
...
static void VideoAddCursor(void) {
	...
	if (VideoState.IsTeletext) {
		CurX *= CHAR_WIDTH;
		CurY = (RelAddr / CRTC_HorizontalDisplayed) * CHAR_HEIGHT;
		//printf("%d\n", CurY);
	}
	else {
	...
	if (CurX + CurSize >= FRAME_WIDTH)
		CurSize = FRAME_WIDTH - CurX;
