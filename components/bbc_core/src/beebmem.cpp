...
#ifdef ESP_PLATFORM
#include "esp_attr.h"
#define BBC_EXT_RAM_BSS EXT_RAM_BSS_ATTR
#else
#define BBC_EXT_RAM_BSS
#endif
...
BBC_EXT_RAM_BSS static unsigned char Roms[16][16384];
...
/* Model B ROM layout used by the upstream roms_bbc.cfg: BASIC in bank 15,
 * DNFS in bank 14. Keeping this byte-buffer entry point avoids filesystem
 * and configuration dependencies on the embedded target. */
void BeebMemInitFromBuffers(const unsigned char *os_rom,
                            const unsigned char *basic_rom,
                            const unsigned char *dfs_rom) {
  memset(WholeRam, 0, sizeof(WholeRam));
  memset(Roms, 0xff, sizeof(Roms));
  for (int bank = 0; bank < 16; ++bank) RomWritable[bank] = 0;

  memcpy(WholeRam + 0xc000, os_rom, 16384);
  memcpy(Roms[15], basic_rom, 16384);
  memcpy(Roms[14], dfs_rom, 16384);
  memcpy(WholeRam + 0x8000, Roms[15], 16384);
  PagedRomReg = 15;
  RomModified = 0;
  SWRamModified = 0;
}
