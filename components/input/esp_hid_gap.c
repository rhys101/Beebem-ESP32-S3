...
static void handle_ble_device_result(const struct ble_gap_disc_desc *disc)
{
    ...
    for (int i = 0; i < fields.num_uuids16; i++) {
        // Accept every standards-compliant HID-over-GATT peripheral, not only
        // the three names used by Espressif's example devices.
        if (ble_uuid_u16(&fields.uuids16[i].u) == BLE_HID_SVC_UUID) {
            is_hid = true;
            break;
        }
    }
    // Logitech's bonded reconnect advertisement can omit the HID UUID even
    // though its pairing advertisement includes it.
    const bool is_pebble = adv_name_len >= 9 &&
                           memcmp(adv_name, "Pebble K3", 9) == 0;
    if (is_hid || is_pebble) {
