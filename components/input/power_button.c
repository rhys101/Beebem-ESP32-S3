...
#define PMU_INTERRUPT_ENABLE_2 0x41
#define PMU_INTERRUPT_STATUS_2 0x49
#define PMU_SHORT_PRESS_BIT (1U << 3)
...
static esp_err_t read_pmu_register(uint8_t reg, uint8_t *value)
{
    return i2c_master_transmit_receive(s_pmu, &reg, 1, value, 1, 100);
}

static esp_err_t write_pmu_register(uint8_t reg, uint8_t value)
{
    const uint8_t data[] = {reg, value};
    return i2c_master_transmit(s_pmu, data, sizeof(data), 100);
}
...
static void button_task(void *argument)
{
    ...
    for (;;) {
        ...
        if (s_use_pmu) {
            uint8_t status = 0;
            if (read_pmu_register(PMU_INTERRUPT_STATUS_2, &status) == ESP_OK) {
                read_failures = 0;
                if ((status & PMU_SHORT_PRESS_BIT) != 0) {
                    // AXP2101 interrupt flags are cleared by writing one. The
                    // latched short-press event is reliable on battery and USB.
                    (void)write_pmu_register(PMU_INTERRUPT_STATUS_2,
                                             PMU_SHORT_PRESS_BIT);
                    ...
                    ESP_LOGI(TAG, "PWR short press from AXP2101; primary action");
                }
            } else if (++read_failures == 50) {
                ESP_LOGW(TAG, "50 consecutive PMU read failures");
                read_failures = 0;
            }
                                 ...
                                 bc32_input_callback_t callback, void *context)
{
    ...
    const i2c_device_config_t pmu_config = {
        .dev_addr_length = I2C_ADDR_BIT_LEN_7,
        .device_address = PMU_ADDRESS,
        .scl_speed_hz = 400000,
    };
    result = i2c_master_bus_add_device(bus, &pmu_config, &s_pmu);
    if (result == ESP_OK) {
        uint8_t enabled = 0;
        result = read_pmu_register(PMU_INTERRUPT_ENABLE_2, &enabled);
        if (result == ESP_OK) {
            result = write_pmu_register(PMU_INTERRUPT_ENABLE_2,
                                        enabled | PMU_SHORT_PRESS_BIT);
        }
        if (result == ESP_OK) {
            // Discard a power-on key event left latched before the emulator's
            // input queue exists, then use PMU events for subsequent presses.
            result = write_pmu_register(PMU_INTERRUPT_STATUS_2,
                                        PMU_SHORT_PRESS_BIT);
        }
        s_use_pmu = result == ESP_OK;
    }
    if (!s_use_pmu) {
        ESP_LOGW(TAG, "AXP2101 PWR event unavailable; using EXIO4 fallback");
    }
    ...
    ESP_LOGI(TAG, "PWR ready via %s (EXIO config 0x%02x, input 0x%02x)",
             s_use_pmu ? "AXP2101 short-press event" : "EXIO4 fallback",
